...
Stowell *et al.* produced point data with area as an attribute, as well as polygons; the difference between the two was only `r signif(abs(solar_uk_area_km2-solar_poly_area_km2),3)`km^2. I ended up using areas calculated from the polygons. This gave the percentage of land used for solar in the UK as a whopping *`r scales::percent(units::drop_units(solar_uk_pct), accuracy=0.001)`*.
